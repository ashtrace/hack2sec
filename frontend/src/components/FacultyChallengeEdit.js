...
const FacultyChallengeEdit = () => {
  ...
  useEffect(() => {
    ...
    const fetchChallenges = async () => {
      try {
        const response = await fetch('http://localhost:1337/api/mychallenges', { headers });

        if (response.status === 200) {
            const data = await response.json();
            setChallenges(data);
          } else if (response.status ===204 ) {
            alert("No Challenges Created !!")
          }
        } catch (error) {
          // Handle fetch error
  ...
  return (
    <div>
      ...
      <Nav_bar pageName="My Challenges" onSetActivePage={setActivePage} />
...
export default FacultyChallengeEdit;
